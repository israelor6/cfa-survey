...
import { ConsumerSurvey } from './components/ConsumerSurvey';
...
function routeFromHash() {
  return window.location.hash.replace(/^#\/?/, '') === 'consumer' ? 'consumer' : 'banks';
}
...
export function App() {
  ...
  useEffect(() => {
    if (!window.location.hash) window.history.replaceState(null, '', `${window.location.pathname}${window.location.search}#/banks`);
    const onHashChange = () => setRoute(routeFromHash());
    window.addEventListener('hashchange', onHashChange);
    return () => window.removeEventListener('hashchange', onHashChange);
  }, []);

  return route === 'consumer' ? <ConsumerSurvey /> : <BankSurvey />;
}
