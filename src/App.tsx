...
import { BLOCKERS, QUESTIONS, ROLE_OPTIONS, ASSET_OPTIONS, BANK_DIRECTORY } from './data/questions';
...
function ContextScreen({ session, onChange, onContinue }: { session: SessionState; onChange: (key: 'bank_name' | 'role' | 'asset_size', value: string) => void; onContinue: () => void }) {
  ...
  const query = (session.respondent.bank_name ?? '').trim().toLowerCase();
  const matches = BANK_DIRECTORY.filter((bank) => !query || bank.name.toLowerCase().includes(query)).slice(0, 6);
  const selectedBank = BANK_DIRECTORY.find((bank) => bank.name.toLowerCase() === query);
  const assetLabel = ASSET_OPTIONS.find((option) => option.value === session.respondent.asset_size)?.label;

  return <main className="context-screen"><div className="context-visual"><span className="overline">Before we begin</span><h1>Make this conversation relevant to your institution.</h1><p>A quick institution match helps us understand the magnitude of the opportunity. Approximate context is enough.</p><div className="context-signal"><span>3–5 min</span><span>7 strategic tradeoffs</span><span>1 CFA thesis</span></div></div><section className="context-card"><label>Bank / institution <small>Search or enter a name</small><div className="bank-combobox"><input value={session.respondent.bank_name ?? ''} onChange={(e) => { onChange('bank_name', e.target.value); setBankSearchOpen(true); }} onFocus={() => setBankSearchOpen(true)} onBlur={() => window.setTimeout(() => setBankSearchOpen(false), 120)} placeholder="Start typing an institution" autoComplete="off" role="combobox" aria-expanded={bankSearchOpen} aria-controls="bank-search-results" />{bankSearchOpen && <div className="bank-search-results" id="bank-search-results" role="listbox">{matches.length > 0 ? matches.map((bank) => <button type="button" role="option" aria-selected={selectedBank?.name === bank.name} key={bank.name} onMouseDown={(event) => event.preventDefault()} onClick={() => { onChange('bank_name', bank.name); setBankSearchOpen(false); }}><span>{bank.name}</span><small>{ASSET_OPTIONS.find((option) => option.value === bank.assetSize)?.label}</small></button>) : <p>No directory match. Keep this name and select the asset range manually.</p>}</div>}</div></label>{selectedBank ? <p className="lookup-success"><span>✓</span><b>{selectedBank.name} matched</b><small>Asset range suggested: {assetLabel}</small></p> : <p className="lookup-note"><span>✦</span> Select a match to populate its asset range automatically.</p>}<label>Your role <select value={session.respondent.role ?? ''} onChange={(e) => onChange('role', e.target.value)}><option value="">Select role</option>{ROLE_OPTIONS.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}</select></label><label>Institution asset size <select value={session.respondent.asset_size ?? ''} onChange={(e) => onChange('asset_size', e.target.value)}><option value="">Select asset size</option>{ASSET_OPTIONS.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}</select></label><button className="primary-button" type="button" onClick={onContinue}>Continue <span>→</span></button><button className="text-button" type="button" onClick={onContinue}>Skip for now</button></section></main>;
}

function Summary({ session, onContact, onSync, onDone, onReset }: { session: SessionState; onContact: (key: 'name' | 'email', value: string) => void; onSync: () => void; onDone: () => void; onReset: () => void }) {
...
export function App() {
  ...
  return <div className="app-shell">
    ...
    <AnimatePresence mode="wait">
      {session.screen === 'intro' && <motion.main className="intro-screen" key="intro" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><header><Brand /><span>Built for banking executives</span></header><div className="intro-content"><span className="intro-signal"><i /> A guided product experience</span><h1>What if your bank actively managed <em>your finances for you</em>?</h1><p>Organize and improve your financial life—without spending your valuable time doing it.</p><button className="primary-button intro-cta" type="button" onClick={() => go('context')}>Start the 3-minute experience <span>→</span></button></div><div className="intro-product"><div className="mini-card"><span>Visible today</span><b>$28.6K</b></div><div className="mini-agent"><span>✦</span><p>CFA found a larger liquidity relationship.</p></div><div className="mini-card accent"><span>With CFA</span><b>$69.4K</b><small>Illustrative household</small></div></div><footer>Customer value <i /> Relationship depth <i /> Household control</footer></motion.main>}
      {session.screen === 'context' && <motion.div key="context" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}><header className="app-header"><Brand /><button className="header-back" type="button" onClick={() => go('intro')}>← Back</button></header><ContextScreen session={session} onChange={(key, value) => setSession((s) => {
        if (key !== 'bank_name') return { ...s, respondent: { ...s.respondent, [key]: value } };
        const match = BANK_DIRECTORY.find((bank) => bank.name.toLowerCase() === value.trim().toLowerCase());
        return { ...s, respondent: { ...s.respondent, bank_name: value, ...(match ? { asset_size: match.assetSize } : {}) } };
      })} onContinue={() => go(1)} /></motion.div>}
      ...
      {session.screen === 'summary' && <motion.div key="summary" initial={{ opacity: 0 }} animate={{ opacity: 1 }}><Summary session={session} onContact={(key, value) => setSession((s) => ({ ...s, respondent: { ...s.respondent, [key]: value } }))} onSync={() => void sync()} onDone={() => window.scrollTo({ top: 0, behavior: 'smooth' })} onReset={() => setConfirmingReset(true)} />{confirmingReset && <div className="modal-backdrop" role="presentation"><div className="confirm-modal" role="dialog" aria-modal="true" aria-labelledby="reset-title"><span className="agent-star">✦</span><h2 id="reset-title">Start another conversation?</h2><p>This clears the current responses and creates a new session ID.</p><div><button className="back-button" type="button" onClick={() => setConfirmingReset(false)}>Cancel</button><button className="primary-button" type="button" onClick={reset}>Start new session</button></div></div></div>}</motion.div>}
    </AnimatePresence>
  </div>;
}
